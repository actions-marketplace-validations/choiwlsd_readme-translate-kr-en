...
| `source-file`    | 번역 소스로 사용되는 README 파일    | 자동 검출 또는 `README.md` |
...
`from` 및 `source-file`가 모두 생략된 경우, CLI는 최신 커밋을 그 부모와 비교한다. 정확히 하나의 표준 README(`README.md`, `README.ko.md` 또는 `README.en.md`)가 변경되면 해당 파일을 소스로 사용하고 콘텐츠에서 해당 언어를 감지한다. 그런 다음 번역된 상대방을 업데이트한다.

Git 이력을 사용할 수 없거나 표준 README가 변경되지 않은 경우 CLI는 `README.md` 및 콘텐츠 기반 언어 검출로 되돌아간다.
...
동일한 커밋에서 복수의 표준 README 파일이 수정되면, CLI는 추측하기보다는 정지한다. `from` 및 `source-file` 둘 다를 명시적으로 지정한다.
